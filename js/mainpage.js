...
startDate.setDate(endDate.getDate() - 30); // 31일의 저주 방지
...
// ⭐️ 강력한 다중 프록시 + 로컬 캐시 엔진
async function fetchXMLWithCache(url, cacheKey, ttlMinutes = 10) {
    ...
    const fetchPromises = proxies.map(async (proxy) => {
        ...
        if (proxy.type === 'json') {
            ...
            if (!data || !data.contents) throw new Error("JSON 누락");
        ...
        if (testDoc.getElementsByTagName("parsererror").length > 0) throw new Error("XML 파싱 에러");
        return xmlString; 
    });

    try {
        const validXmlString = await Promise.any(fetchPromises);
        localStorage.setItem(cacheKey, JSON.stringify({
            timestamp: Date.now(),
            xmlString: validXmlString
        }));
        return new DOMParser().parseFromString(validXmlString, "text/xml");
    } catch (aggregateError) {
        if (cachedData) {
            return new DOMParser().parseFromString(JSON.parse(cachedData).xmlString, "text/xml");
        }
...
function fetchAllData() {
    const sDateNode = document.getElementById('startDate');
    const eDateNode = document.getElementById('endDate');
    
    if(sDateNode?.value) stdateStr = sDateNode.value.replace(/-/g, '');
    if(eDateNode?.value) eddateStr = eDateNode.value.replace(/-/g, '');
...
async function fetchBoxOffice() {
    ...
    let url = `http://www.kopis.or.kr/openApi/restful/boxoffice?service=${API_KEY}&stdate=${stdateStr}&eddate=${eddateStr}`;
    if (currentBoxOfficeGenre) url += `&catecode=${currentBoxOfficeGenre}`;

    try {
        const cacheKey = `data_boxoffice_${currentBoxOfficeGenre}`;
        const xmlDoc = await fetchXMLWithCache(url, cacheKey, 10);
            ...
            return `
                <div onclick="location.href='detail.html?id=${id}'" class="w-[240px] md:w-[280px] flex-shrink-0 cursor-pointer group snap-start">
                    <div class="relative rounded-2xl overflow-hidden mb-4 shadow-sm">
                        <img src="${poster}" alt="${title}" loading="lazy" class="w-full aspect-[3/4] object-cover hover-scale transition-transform duration-500 group-hover:scale-110" onerror="this.onerror=null; this.src='https://via.placeholder.com/300x400?text=No+Image';">
...
async function fetchPerformanceList() {
    ...
    let url = `http://www.kopis.or.kr/openApi/restful/pblprfr?service=${API_KEY}&stdate=${stdateStr}&eddate=${eddateStr}&cpage=1&rows=10`;
    if (currentListGenre) url += `&shcate=${currentListGenre}`;
            ...
            return `
                <div onclick="location.href='detail.html?id=${id}'" class="cursor-pointer group flex flex-col">
                    <div class="relative rounded-xl overflow-hidden mb-3 shadow-sm border border-gray-100">
                        <img src="${poster}" alt="${title}" loading="lazy" class="w-full aspect-[3/4] object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.onerror=null; this.src='https://via.placeholder.com/300x400?text=No+Image';">
