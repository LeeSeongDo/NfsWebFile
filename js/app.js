...
// XML 안전 추출 헬퍼 함수 (에러 방지)
const getText = (element, tagName, defaultValue = '') => {
    const node = element.getElementsByTagName(tagName)[0];
    return node ? node.textContent : defaultValue;
};

// 공통 XML Fetch 함수 (CORS 프록시 및 에러 핸들링 통합)
// [최종 진화형 fetchXML] 다중 프록시 우회 및 에러 복구 시스템 적용
async function fetchXML(url) {
    // 3중 안전장치: 신뢰도가 높은 순서대로 프록시 서버 리스트 구성
    const proxies = [
    ...
    // 프록시 배열을 순회하며 하나라도 성공할 때까지 재시도
    for (let i = 0; i < proxies.length; i++) {
        const proxyUrl = proxies[i];
        try {
            // 타임아웃 또는 서버 에러 발생 시 여기서 catch로 넘어감
            ...
            // 성공 시 즉시 데이터 반환 (루프 탈출)
            return xmlDoc;

        } catch (error) {
            console.warn(`[프록시 시도 ${i + 1}/${proxies.length} 실패] ${proxyUrl} - 사유: ${error.message}`);
            lastError = error;
            // 실패하더라도 에러를 띄우지 않고 다음 프록시 서버(for 루프 다음 단계)로 은밀히 넘어갑니다.
        }
    }

    // 3개의 프록시가 모두 죽었거나 KOPIS 서버 자체가 완전히 다운된 최악의 경우
...
async function fetchAllData() {
    ...
    // 두 API를 병렬로 호출하되, 내부에서 상세 API를 호출하지 않으므로 매우 빠름
...
async function fetchBoxOffice(stdate, eddate) {
    ...
    let url = `http://www.kopis.or.kr/openApi/restful/boxoffice?service=${API_KEY}&stdate=${formatDate(stdate)}&eddate=${formatDate(eddate)}`;
...
async function fetchPerformanceList(stdate, eddate) {
    ...
    let url = `http://www.kopis.or.kr/openApi/restful/pblprfr?service=${API_KEY}&stdate=${formatDate(stdate)}&eddate=${formatDate(eddate)}&cpage=1&rows=15`;
